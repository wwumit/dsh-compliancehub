...
const CATALOG_URL = 'https://wwumit.github.io/skills-catalog/catalog.json'
...
async function main(): Promise<void> {
  ...
  const one = await ctx.skills.get('ccpa-check', { cwd: '/Users/wuwei/Documents/DSH' })
  console.log(`✅ ctx.skills.get('ccpa-check') → ${one ? `正文 ${one.content.length} 字节，前 60 字: ${one.content.slice(0, 60).replace(/\n/g, ' ')}` : 'undefined ❌'}`)
